...
<%--	<!--[if IE]><script type="text/javascript" src="<%=request.getContextPath() %>/js/excanvas_r73.js"></script><![endif]-->--%>
	...
	<div id="tree_controls">
		...
		<% if (hasMultipleOrders) { %>
			...
			<%
			int i = 0;
			for (Map<String, Object> node: orderList) {
			    %><option value="<%=node.get("taxonId")%>" <%=(i==0)?"selected='selected'":"" %>><%=node.get("name")%></option><%
			}
			i++;
			%>
			</select>
		<%
            ...
            Map<String, Object> node = orderList.get(0);

            %><%=node.get("name") %><input type="hidden" id="tree_select_list" value="<%=node.get("taxonId") %>"/><%
		}
		%>
		</div>
		...
		<div style="float:left;width:25%">
			<a id="nwk_link" href="/patric-phylogeny/resources/readNWK.jsp?taxonID=<%=taxonId %>" target="_blank">Download tree as Newick file</a>
		</div>
		<div>
...
<%
	...
	%><p>Phylogeny tree is not available for this taxonomy/genome currently. Please <a href="http://enews.patricbrc.org/contact-us/" target="_blank">contact us</a> for further questions.</p><%
}
%>
