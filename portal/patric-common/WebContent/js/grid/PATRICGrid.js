Ext.define('Ext.grid.PATRICGrid', {
	...
	setSortDirectionColumnHeader : function(incomingstate) {
		...
		var columns = this.columns, id, i, j, sort = null, direction = null, property = $Page.getPageProperties(), hash = property.hash, 
		which = hash.aT ? hash.aT : 0, 
		state = incomingstate || (property.stateId && Ext.state.Manager.get(property.stateId[which])? Ext.state.Manager.get(property.stateId[which]).sort : property.sort[which]);
			...
			for ( i = 0; i < columns.length; i++) {
				...
				if (Object.prototype.toString.call(sort) === "[object Array]") {
					for ( j = 0; j < sort.length; j++) {
						...
						if (columns[i].dataIndex == sort[j] && id != undefined)
							id.addCls('x-column-header-sort-' + direction[j]);
					}
				} else {
					sort = sort.replace(/_sort/g, "");
					if (columns[i].dataIndex == sort && id != undefined)
