...
function catchLocusTags(colID, rowID, ajaxHttp) {
	...
	for ( i = 0; i < locusList.length; i++) {
		...
		if (locusList[i].alt_locus_tag != undefined) {
			text += ",  " + locusList[i].alt_locus_tag;
		}
	}
		...
		if (0 < membersCount) {
			...
			download = new Ext.Button({
				...
				menu : [{
					...
					icon : '/patric/images/toolbar_excel.png',
					handler : function() {
						MediatorDownloadMultiple(rows, columns, 'xlsx');
						selectPop.hide();
					}
				}]
			});
				...
				for ( k = 0; k < affectedRows.length; k++) {
					for ( l = 0; l < rows.length; l++) {
						if (affectedRows[k] == rows[l].rowID) {
							column += (new String(rows[l].rowLabel)).trim() + "\t";
							distribution += parseInt(cols[j].distribution.charAt(l * 2) + cols[j].distribution.charAt(l * 2 + 1), 16) + "\t";
						}
					}
				}
				if (i == 0)
					data += "Genomes/Protein Families\t" + column + "\n" + (new String(cols[j].colLabel)).trim() + "\t" + distribution + "\n";
				else
					data += (new String(cols[j].colLabel)).trim() + "\t" + distribution + "\n";
			}
		}
	}

	Ext.getDom("fTableForm_Cell").action = "/patric-proteinfamily/jsp/GetDetailTable.jsp";
