...
function getGenomeNames(windowID, stateObject) {
	...
	Ext.getDom("information_panel").innerHTML = "<p>For a description of what you can do from this Protein Families Sorter page, please see our " + "<a href='http://enews.patricbrc.org/protein-family-sorter/'>Protein Family Sorter User Guide.</a></p>";
...
function setSyntenyOrder(windowID, genomeId) {
	...
	Ext.Ajax.request({
		...
		failure: function(){
			Ext.MessageBox.alert("Error", "Data is not available now. Please try again later or contact PATRIC team (http://enews.patricbrc.org/contact-us/).");
...
function setTopOrtho(windowID, fileType) {
	...
	for (var i = 0; i < tableData.length; i++) {
		...
		for (var j = 1; j < column_order.length; j++) {
		  var str = new String(rowData[column_order[j]]);
			tablePass += "\t" + str.trim();
		}
