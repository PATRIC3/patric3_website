...
function renderMapTableGene(value, metadata, record, rowIndex, colIndex, store){
	...
	if (rgc == gc) {
		if(cType && (cType == "taxon" || cType == "genome")) {
			return Ext.String.format('<a href=\"CompPathwayTable?cType={0}&amp;cId={1}&amp;algorithm={2}&amp;ec_number={3}&amp;map=<%=map%>\" target=\"_blank\"><b>{4}</b></a>',Ext.getDom("cType").value, Ext.getDom("cId").value, algo, record.data.ec_number, value);
		}
		else {
		...
		if(cType && (cType == "taxon" || cType == "genome")) {
			return Ext.String.format('<a href=\"CompPathwayTable?cType={0}&amp;cId={1}&amp;algorithm={2}&amp;ec_number={3}&amp;map=<%=map%>\" target=\"_blank\">{4}</a>',Ext.getDom("cType").value, Ext.getDom("cId").value, algo, record.data.ec_number, value);
		}
		else {
