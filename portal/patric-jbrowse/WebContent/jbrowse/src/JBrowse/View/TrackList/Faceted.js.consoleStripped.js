...
var RowSelector = declare("dojox.grid.cells.RowSelector", gridCells._Widget, {
	...
	_pageDestroyed: function(pageIndex){
		...
		//0 && console.log("Page ",pageIndex, " destroyed, Map=",this.map);
	},
	destroy: function(){
		...
		//0 && console.log('Single(Multiple)RowSelector.destroy() executed!');
	}
});
		...
		array.forEach(this.structure, function(view){
			...
			if(cells && cells.length > 0 && !rowSelectCellAdded){
				...
				if(firstRow[0] && firstRow[0].isRowSelector){
					0 && console.debug('addRowSelectCell() - row selector cells already added, return.');
				...
				array.forEach(this.cells, function(cell, i){
					if(cell.index >= 0){
						cell.index += 1;
						//0 && console.debug('cell '+ (cell.index - 1) +  ' is updated to index ' + cell.index);
					}else{
