            ...
            if( error ) {
                ...
                var refLen = refseq.end - refseq.start;
                 if( stats._statsSampleFeatures >= 300 || interval * 2 > refLen || error ) {
                     0 && console.log( 'Store statistics: '+(this.source||this.name), stats );
 ...
 */
{
    ...
    fetch: function(chr, min, max, featCallback, endCallback, errorCallback ) {
        ...
        //0 && console.log( chr, min, max, chunks.toString() );
    ...
    _readChunk: function( chunk, callback ) {
        var thisB = this;
        var features = [];
        // 0 && console.log('chunk '+chunk+' size ',Util.humanReadableNumber(size));
