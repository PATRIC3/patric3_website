...
public class DBSummary {
	 ...
	 */
	public ResultType getTaxonomyNodeForGenomeSelector(Map<String, String> key) {
	 ...
	 */
	public List<ResultType> getGenomeListForGenomeSelector(Map<String, String> key) {
	 ...
	 */
	public List<ResultType> getTaxonomyTreeForGenomeSelector(Map<String, String> key) {
		String sql = "SELECT node_level, node_left, node_right from cas.ncbiclassification where ncbi_taxon_id = ?";
	...
	public ArrayList<ResultType> getFIGFamConservDist(int taxonId) {
		String sql = "select grp, count(*) cnt from " 
				+ "	(select a.name, ceil(a.gcnt/b.gcnt*10) grp from "
				+ "		(select name, count(distinct(genome_info_id)) gcnt " 
				+ "			from app.figfamsummary " 
				+ "			where ncbi_tax_id in (" + getTaxonIdsInTaxonSQL(":taxonId") + ") " 
				+ "			group by name) a, " 
				+ "		(select count(genome_info_id) gcnt "
				+ "			from app.genomesummary " 
				+ "			where rast=1 and ncbi_tax_id in (" + getTaxonIdsInTaxonSQL(":taxonId") + ") " 
				+ "		) b " 
				+ "	) "
				+ "group by grp " 
				+ "order by grp";
	...
	public ResultType getFIGFamStat(int taxonId) {
		...
		sql = "select count(*) cnt from (select a.name, ceil(a.gcnt/b.gcnt*10) grp from "
				...
				+ "	where a.gcnt > 0.95 * b.gcnt ) group by grp";
		q = session.createSQLQuery(sql);
		q.setInteger("taxonId", taxonId);
		q.addScalar("cnt", Hibernate.INTEGER).setCacheable(true);
		cnt = q.uniqueResult();
