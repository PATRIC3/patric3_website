...
public class DocumentMapBinder {
	...
	public <T> List<T> getBeans(Class<T> clazz, List<Map> solrDocList) {
		List<DocField> fields = getDocFields(clazz);
		List<T> result = new ArrayList<>(solrDocList.size());

		for (Map sdoc : solrDocList) {
			result.add(getBean(clazz, fields, sdoc));
		}
		return result;
	}

	public <T> T getBean(Class<T> clazz, Map solrDoc) {
	...
	private static class DocField {
		...
		private void storeType() {
			...
			else if (type.isPrimitive() && type.getName().equals("float") || type == Float.class) {
				isFloat = true;
			}
		}
		...
		private Object getFieldValue(Map<String, Object> solrDocument) {
			// Object fieldValue = solrDocument.getFieldValue(name);
			Object fieldValue = solrDocument.get(name);
		...
		<T> void inject(T obj, Map sdoc) {
			...
			if (isArray && !isContainedInMap) {
				...
				try {
					set(obj, dateFormat.parse(val.toString()));
				}
				...
				if (val instanceof Float) {
					...
					set(obj, ((Double) val).floatValue());
				}
				else if (val instanceof Integer) {
					set(obj, ((Integer) val).floatValue());
				}
			}
			else {
				set(obj, val);
		...
		public Object get(final Object obj) {
			if (field != null) {
				...
				throw new BindingException(
						"Missing getter for field: " + name + " -- You can only call the 'get' for fields that have a field of 'get' method");
			}
