	...
	window.FigFamChart = FigFamChart = (function(_super) {
		...
		*/
		FigFamChart.prototype.prepChart = function(response) {
			...
			*/
			...
			this.pf_y_scale = d3.scale.linear().range([0, s_chartCanvas.height]).domain([this.max, 0]);
			...
			*/
			...
			this.chart.append("g").attr("transform", "translate(" + drawtargetParams.margins[3] + "," + drawtargetParams.margins[0] + ")").call(yAxis).attr("class", "y axis");
		...
		*/
		...
		FigFamChart.prototype.barheight = function(value, total) {
			...
			*/
			if (this.normalize) {
				return this.chartheight - this.pf_y_scale((value / total) * 100);
			}
			else {
				return this.chartheight - this.pf_y_scale(value);
			}
		};
		...
		*/
		FigFamChart.prototype.updateProteinFamilies = function(incomingDataset, callback) {
			...
			// update axis
			if (this.normalize) {
				this.pf_y_scale = d3.scale.linear().range([0, this.chartheight]).domain([100, 0]);
			}
			else {
				this.pf_y_scale = d3.scale.linear().range([0, this.chartheight]).domain([this.max, 0]);
			}
...
/*
	PathwayChart
	customized for pathway conservation distribution. need refactoring.
*/
...
(function() {
	...
	window.PathwayChart = PathwayChart = (function(_super) {
		__extends(PathwayChart, _super);

		function PathwayChart(props) {
			var self;
			PathwayChart.__super__.constructor.call(this, props);
			self = this;
			d3.json(this.p.datafile, function(data) {
				return self.prepChart(data.conservation);
			});
		}
		...
		/* 
			Set up and build the chart.
		*/
		PathwayChart.prototype.prepChart = function(response) {
			var $targ, barset, chart, chartTitle, dataset, datasetToggle, datum, desc, drawtarget, drawtargetParams, i, legendYPos, maxvalues, scaleToggle, self, sortToggle, tabTitle, yAxis, _fn, _i;
			chartTitle = response.chart_title;
			tabTitle = response.tab_title;
			desc = response.chart_desc;
			dataset = response.data;
			this.currentSort = "index";
			this.currentDataset = "dist";
			this.ascendSort = true;
			this.normalize = false;
			$(this.p.headerSelector).text(chartTitle);
			$(this.p.descSelector).text(desc);
			...
			*/
			...
			this.pf_y_scale = d3.scale.linear().range([0, this.canvasSize.height]).domain([this.max, 0]);
			...
			*/
			...
			yAxis = d3.svg.axis().scale(this.pf_y_scale).orient("left").tickPadding(0).tickSize(0);
			this.chart.append("g").attr("transform", "translate(" + drawtargetParams.margins[3] + "," + drawtargetParams.margins[0] + ")").call(yAxis).attr("class", "y axis");
			...
			*/
			_fn = function(barset) {
				self.bars.append("rect").attr("class", "block-" + barset).attr("y", function(d, i) {
					...
					bardata = d[self.currentDataset];
					ancestorheight = self.barheight(d3.sum(bardata.slice(0, barset)), d.total) || 0;
					return Math.round(self.canvasSize.height - self.barheight(bardata[barset], d.total) - ancestorheight);
				}).attr("x", function(d, i) {
					return self.barPosition(i);
				}).attr("width", function() {
					return self.barWidth();
			...
			*/
			...
			return this.bars.append("text").text(function(d, i) {
				return d.label;
			}).attr("y", Math.round(this.canvasSize.height - 11)).attr("x", function(d, i) {
				return self.textPosition(i);
			}).attr("transform", function(d, i) {
				var x, y;
				y = Math.round(self.canvasSize.height - 11);
				x = self.textPosition(i);
				return "rotate(270," + x + "," + y + ")";
			}).attr("dy", ".35em");
		};
		...
		/*
			Used to dynamically calculate the text postion based bar size.
		*/
		PathwayChart.prototype.textPosition = function(index) {
			return Math.floor((this.full_barWidth * index) + this.drawn_barWidth);
		};
		...
		/*
			Figure out the X position of a given bar based on index
		*/
		PathwayChart.prototype.barPosition = function(index) {
			return Math.floor(this.full_barWidth * index + this.center_correction);
		};
		...
		/*
			...
			but it's possible that we would want more complex logic here later.
		*/
		PathwayChart.prototype.barWidth = function() {
			return Math.floor(this.drawn_barWidth);
		};

		PathwayChart.prototype.barheight = function(value, total) {
			if (this.normalize) {
		...
		/*
			...
			and dataset swaps.
		*/
		PathwayChart.prototype.updateProteinFamilies = function(incomingDataset, callback) {
			var processed, self;
			if (incomingDataset !== this.currentDataset) {
				this.currentDataset = incomingDataset;
			}
			...
			this.pf_y_scale = d3.scale.linear().range([0, this.canvasSize.height]).domain([maxValue, 0]);
			this.yAxis = d3.svg.axis().scale(this.pf_y_scale).orient("left").tickPadding(0).tickSize(0);
			this.chart.select("g.y").transition().duration(600).call(this.yAxis);
			
			// update bars
			this.bars.select("rect.block-0").transition().duration(600).attr("y", function(d, i) {
				var bardata = d[self.currentDataset];
				return self.canvasSize.height - self.barheight(bardata[0], d.total);
			}).attr("height", function(d, i) {
				var bardata;
				bardata = d[self.currentDataset];
				return self.barheight(bardata[0], d.total);
			...
			this.bars.select("rect.block-1").transition().duration(600).attr("y", function(d, i) {
				var bardata = d[self.currentDataset];
				return self.canvasSize.height - self.barheight(bardata[0], d.total) - self.barheight(bardata[1], d.total);
			}).attr("height", function(d, i) {
				var bardata = d[self.currentDataset];
				return self.barheight(bardata[1], d.total);
			});
			
			this.bars.select("rect.block-2").transition().duration(600).attr("y", function(d, i) {
				var bardata = d[self.currentDataset];
				return self.canvasSize.height - self.barheight(bardata[0], d.total) - self.barheight(bardata[1], d.total) - self.barheight(bardata[2], d.total);
			}).attr("height", function(d, i) {
				var bardata;
				bardata = d[self.currentDataset];
				return self.barheight(bardata[2], d.total);
			});
			
			this.bars.select("rect.block-3").transition().duration(600).attr("y", function(d, i) {
				var bardata = d[self.currentDataset];
				return self.canvasSize.height - self.barheight(bardata[0], d.total) - self.barheight(bardata[1], d.total) - self.barheight(bardata[2], d.total)- self.barheight(bardata[3], d.total);
			}).attr("height", function(d, i) {
				var bardata = d[self.currentDataset];
				return self.barheight(bardata[3], d.total);
			});
			
			return this.bars.select("rect.block-4").transition().each('end', function() {
				processed++;
				if (processed === self.data.length) {
					return callback();
				}
				...
				var bardata = d[self.currentDataset];
				return self.canvasSize.height - self.barheight(bardata[0], d.total) - self.barheight(bardata[1], d.total) - self.barheight(bardata[2], d.total) - self.barheight(bardata[3], d.total) - self.barheight(bardata[4], d.total);
			}).attr("height", function(d, i) {
				var bardata = d[self.currentDataset];
				return self.barheight(bardata[4], d.total);
			});
		};
		...
		/*
			...
			happens a little later.
		*/
		PathwayChart.prototype.sortProteinFamilies = function(sortCriteria) {
			var allowedSorts, self;
			if (sortCriteria == null) {
				sortCriteria = null;
			}
			...
			*/
			...
			} else if (this.currentSort === "conserved") {
				this.bars.sort(function(a, b) {
					var aValues, bValues, orderCode;
					aValues = self.barheight(a[self.currentDataset][0], a.total);
					bValues = self.barheight(b[self.currentDataset][0], b.total);
			...
			*/
			...
			this.bars.select("rect.block-0").transition().duration(600).delay(function(d, i) {
				...
				var bardata;
				bardata = d[self.currentDataset];
					return Math.round(self.canvasSize.height - self.barheight(bardata[0], d.total));
			});
			/*
				Animate the 1-element bars into position. Again, y-value
				might be updated if our focus is on a different bar than last time.
			*/
			...
			this.bars.select("rect.block-1").transition().duration(600).delay(function(d, i) {
				...
				var bardata;
				bardata = d[self.currentDataset];
					return Math.round(self.canvasSize.height - self.barheight(bardata[0], d.total) - self.barheight(bardata[1], d.total));
			});
			
			this.bars.select("rect.block-2").transition().duration(600).delay(function(d, i) {
				return 15 * i;
			}).attr("x", function(d, i) {
				return self.barPosition(i);
			}).attr("y", function(d, i) {
				var bardata;
				bardata = d[self.currentDataset];
					return Math.round(self.canvasSize.height - self.barheight(bardata[0], d.total) - self.barheight(bardata[1], d.total) - self.barheight(bardata[2], d.total));
			});
			
			this.bars.select("rect.block-3").transition().duration(600).delay(function(d, i) {
				return 15 * i;
			}).attr("x", function(d, i) {
				return self.barPosition(i);
			}).attr("y", function(d, i) {
				var bardata;
				bardata = d[self.currentDataset];
					return Math.round(self.canvasSize.height - self.barheight(bardata[0], d.total) - self.barheight(bardata[1], d.total) - self.barheight(bardata[2], d.total) - self.barheight(bardata[3], d.total));
			});
			
			this.bars.select("rect.block-4").transition().duration(600).delay(function(d, i) {
				return 15 * i;
			}).attr("x", function(d, i) {
				return self.barPosition(i);
			}).attr("y", function(d, i) {
				var bardata;
				bardata = d[self.currentDataset];
					return Math.round(self.canvasSize.height - self.barheight(bardata[0], d.total) - self.barheight(bardata[1], d.total) - self.barheight(bardata[2], d.total) - self.barheight(bardata[3], d.total) - self.barheight(bardata[4], d.total));
			});
			/*
				Animate text into position. Text never changes y position once placed.
			*/
	...
	window.HistogramChart = HistogramChart = (function(_super) {
		...
		*/
		...
		HistogramChart.prototype.prepChart = function() {
			...
			canvas.append("text").attr("transform", "rotate(-90)").attr("x", 0 - (chartwidth/2)).attr("y", -50).text("# of Protein Families");
			canvas.append("text").attr("transform", "translate(60," + (chartheight + 35) + ")").text("Conservation Across %Genomes");
...
(function() {
	...
	window.HorizontalBarChart = HorizontalBarChart = (function(_super) {
		...
		HorizontalBarChart.prototype.prepChart = function() {
			...
			canvas.append("g").attr("class", "x axis").attr("transform", "translate(0," + chartheight + ")").call(xAxis);
			canvas.append("text").attr("transform", "translate(" + d3.round(chartwidth/3) + "," + (chartheight + 35) + ")").text("Experiments");
			canvas.append("g").attr("class", "y axis").call(yAxis);
		...
		HorizontalBarChart.prototype.updateChart = function(newIdx, callback) {
			...
			// update clickHandler
			this.bars.select("text").on("click", function(d, i) {
				var meta = {
					"clickTarget": this,
					"chartTarget": self.p.target,
					"linkBase": self.chartdata[self.currentDataIndex].link
